...
namespace MrCrusher.Framework.GameConditions {
    ...
    public class GameRunningConditions {
        ...
        public static void AddNewEnemySoldiersAtRandomPosition(int maxNumberOfSoldiers) {
            ...
            foreach (var newEnemy in newEnemies.OfType<RotatingObject>()) {
                var newPlayerAsTarget = GameEnv.RandomGameObjectControlledByAPlayer;
                if (newPlayerAsTarget != null) {
                    newEnemy.CurrentMission = new MoveToPointMission(newPlayerAsTarget.PositionCenter);
                }
            }
        }
